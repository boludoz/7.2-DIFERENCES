...
Func BuilderBaseReport()
	...
	Local $sAttackDirectory = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\Resources\Attacks"
	Local $aResult = QuickMIS("CX", $sAttackDirectory, 20, 625, 110, 650, True)
	$g_iAttacksAvailable = UBound($aResult)

	; LOG
	Setlog("Builder Base Report", $COLOR_INFO)
	Setlog("[G]: " & _NumberFormat($g_iGoldBB) & " [E]: " & _NumberFormat($g_iElixirBB) & " [T]: " & _NumberFormat($g_iTrophiesBB), $COLOR_SUCCESS)
	Setlog("[Master Builder]: " & $g_aBuilder[0] & "/" & $g_aBuilder[1], $COLOR_SUCCESS)
	If $g_iAttacksAvailable <> 0 Then
	  Setlog("Hey, Chief!! You have " & $g_iAttacksAvailable & " available attacks!", $COLOR_SUCCESS)
