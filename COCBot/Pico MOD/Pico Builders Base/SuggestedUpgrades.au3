...
Func NewBuildings($aResult)
	...
	If UBound($aResult) = 3 And $aResult[2] = "New" Then
		...
		Local $directoryZero = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\AutoUpgrade\NewBuildings\Shop"
		Local $directoryClock = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\AutoUpgrade\NewBuildings\Clock"
		Local $directoryInfoIcon = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\AutoUpgrade\NewBuildings\Slot"
		...
		If UBound($ClocksCoordinates) > 0 Then
			...
			For $i = 0 To UBound($ClocksCoordinates) - 1
				...
				If $g_iDebugSetlog then Setlog("[x]: " & $x & " [y]: " & $y & " [x1]: " & $x1 & " [y1]: " & $y1, $COLOR_DEBUG)
				If QuickMIS("BC1", $directoryZero, $x, $y, $x1, $y1, $Screencap, $Debug) Then
					...
					If Not QuickMIS("BC1", $directoryInfoIcon, $x, $y, $x1, $y1, $Screencap, $Debug) Then
						...
						Local $directory = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\AutoUpgrade\NewBuildings\Yes"
						If QuickMIS("BC1", $directory, 150, 150, 650, 550, $Screencap, $Debug) Then
							...
							Return True
						Else
							Local $directory = @ScriptDir & "\imgxml\Resources\PicoBuildersBase\AutoUpgrade\NewBuildings\No"
							If QuickMIS("BC1", $directory, 150, 150, 650, 550, $Screencap, $Debug) Then
					...
					Else
						If $i = UBound($ClocksCoordinates) - 1 Then
							If $g_iDebugSetlog then Setlog("Slot without enough resources![1]", $COLOR_DEBUG)
							Click(820, 38, 1) ; exit from Shop
							ExitLoop
						EndIf
						ContinueLoop
					EndIf
				Else
					If $g_iDebugSetlog then Setlog("Slot without enough resources![2]", $COLOR_DEBUG)
